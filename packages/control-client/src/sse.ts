// Reusable SSE consumer for streaming API responses. Uses fetch() +
// ReadableStream to parse `data: {...}\n\n` lines and route parsed events to
// typed handler callbacks. `consumeSSE` takes a full URL (base-agnostic
// primitive); `consumeApiSSE` is the base + header-aware wrapper for `/api/...`
// paths.

import { apiUrl, getRequestHeaders } from './config'
...
/**
 * Base + header-aware SSE consumer for `/api/...` paths. Resolves the path
 * against the configured base and merges the injected auth/tenant headers, so a
 * thin client streaming from a remote host works with the same call.
 */
export function consumeApiSSE(
  path: string,
  options: RequestInit,
  handlers: SSEHandlers,
): AbortController {
  return consumeSSE(
    apiUrl(path),
    { ...options, headers: { ...getRequestHeaders(), ...(options.headers ?? {}) } },
    handlers,
  )
}
