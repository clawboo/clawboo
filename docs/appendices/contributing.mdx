...
<Tip>
Running the full gate locally avoids review back-and-forth. At minimum, every PR must pass `pnpm build`, `pnpm lint`, `pnpm typecheck`, and `pnpm test`. CI also runs `pnpm e2e` and CodeQL code scanning on every PR, so keep those green too; `pnpm e2e` needs a built workspace and a Chromium download (`pnpm build`, then `pnpm exec playwright install chromium`).
</Tip>
