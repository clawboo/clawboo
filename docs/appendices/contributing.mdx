...
pnpm lint                               # ESLint across all packages, plus the docs frontmatter check
...
`mint broken-links` only validates links, so it will not catch that; a full `mint dev` build will. `pnpm check:docs` catches it too, without needing the Mintlify CLI: it is the `docs` package's `lint` script, so it also runs under `pnpm lint` and in CI's Lint job on every PR. The repo's [`docs/README.md`](https://github.com/clawboo/clawboo/blob/main/docs/README.md) covers the rest: the directory layout, the hand-maintained `docs.json` navigation, and local preview.
