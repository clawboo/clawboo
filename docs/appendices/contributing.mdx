...
pnpm verify:connectors                  # connector catalog consistency gate (offline)
pnpm catalog:build                      # rebuild catalog/dist and the compiled marketplace seed
pnpm catalog:verify                     # marketplace content gate (offline)
...
- **`pnpm verify:connectors`** checks the committed connector catalog for internal consistency. It is offline, and it is the gate CI and the release path run.
- **`pnpm catalog:build`** rebuilds `catalog/dist/**` and the compiled marketplace seed from the packs in `catalog/packs/**` (436 agents, 85 teams across nineteen packs). **`pnpm catalog:verify`** is the gate: every content rule, then a check that the committed artifacts are byte-for-byte what a rebuild would write. Both are offline. A pull request touching only `catalog/` skips the full CI matrix and runs `catalog-ci.yml` instead; see [the marketplace catalog reference](/reference/marketplace-catalog).
...
Releases are automated and maintainer-only: when changesets land on `main`, the `publish.yml` workflow opens a "Version Packages" PR; merging it bumps versions, updates changelogs, and publishes the `clawboo` CLI. No manual `npm publish`. The workflow re-runs the whole PR gate before publishing (`build`, `lint`, `typecheck`, `test`, then `assemble` and the `pnpm test:clean-install` gate), so a release cannot skip a check a pull request had to pass. Every check on that path is offline, so an upstream outage cannot block a release. See [Release process](/internals/release-process) for the full pipeline.
