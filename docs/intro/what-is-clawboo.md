...
The gate is **intrinsic to the board state machine**, not an opt-in step; any transition to `done` with a non-promotable verdict is rejected for _every_ caller (the REST route, the Tasks-MCP tool, the orchestrator), the only escape being an explicit, audited `humanOverride`. When the fix loop runs out of budget the verdict is recorded as `completed_with_debt` and the task is parked `blocked` for a human rather than landing `done`; only a clean `pass` promotes, and the delegator is told so it is never a quiet stall.
