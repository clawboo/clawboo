...
- **The key doesn't verify** → nothing is stored and you stay on this step, with the reason inline ("Invalid API key.", "Could not reach anthropic.", …) next to the field you can fix. Picking **Ollama** while it isn't running shows a start hint (`ollama serve`) instead. If the check is failing because _this machine_ can't reach the provider rather than because the key is wrong, **Continue anyway** stores it and moves on.
