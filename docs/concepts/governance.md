...
The **children cap** is the board-write counterpart to fan-out: fan-out bounds one turn, this bounds one parent for its whole life. It is enforced by the board's capped create path — the check and the insert share one `BEGIN IMMEDIATE` transaction, so two attached runtimes racing the same parent cannot both slip past the ceiling — and reached through the Tasks MCP create tools, where an attached runtime creates rows unsupervised. It refuses with a tool-error the caller must not retry. Dropped (soft-deleted) children fall out of the count, which is the user's way to free a slot; `done` and `cancelled` ones still count, because they are still rows on the board and the alternative would be trivially evadable by an agent that cancels as it creates. The refusal goes back to the calling model only; it is not recorded in the governance audit log.
