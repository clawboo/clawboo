...
- **Single implicit tenant today.** `memory_facts` and `memory_procedures` carry a `tenant_id` column, but it is a dormant seam; no per-tenant filtering is active in v0.3.0. The store's tenant scoping path exists; it is not wired to a tenant boundary yet.
