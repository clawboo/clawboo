...
<Danger>
Because there is no migration ladder, a schema change at this stage is a **hard reset** of the local DB, not an in-place upgrade. In v0.3.0 this is acceptable; it is the part of this invariant most likely to change when real data needs preserving across schema versions.
</Danger>
...
The idempotent schema (7) buys a zero-friction fresh install, `createDb` produces a usable database with no migration step, at the cost of no in-place upgrade path, a deliberate v0.3.0 trade-off.
...
- **The schema invariant will likely evolve again.** "No migration ladder, hard reset on change" is a v0.3.0 posture. Preserving real data across schema versions is a planned future requirement.
