...
The orchestrator is **structured-first**. It reads only typed lifecycle signals, never prose:

- a **`delegate` tool-call** (how a native leader hands work off) or a **`sessions_send` tool-call** (OpenClaw's built-in), each resolved to a target against the team roster, and
...
On a server-orchestrated team, the OpenClaw adapter ignores the per-message tool blocklist, so tool-level enforcement alone is not enough. The orchestrator therefore injects the same contract, the `<delegate>` syntax plus an explicit "coordinate the existing team, and never spawn sub-agents" rule, into **every OpenClaw agent's volatile per-turn context** (`buildServerTeamContext`) on every OpenClaw run. A vanilla OpenClaw agent that never saw this brief reaches for its native `sessions_spawn` / `sessions_yield` tools and launches its own throwaway sub-agents, bypassing the real team and the board.
</Note>
...
The orchestrator core is a pure, framework-free function (`createBoardOrchestrator`) holding small in-memory maps that bridge a session to its task. It runs **server-side, as one long-lived instance per team** (`getTeamOrchestrator`): it observes each participant's [RuntimeEvent](/appendices/glossary) stream, feeds events to the core, routes deliveries through a nudge-queue, and writes board mutations that stream to the client. Because the engine lives on the server and is keyed to the durable board, a cascade runs to completion with no client connected and survives a tab close or a server restart, and hydrated history is never replayed as new work. The browser is a thin client: it posts the user's message (`POST /api/teams/:id/chat`), renders the live stream over Server-Sent Events (`/chat/stream`), and stops via REST (`/chat/stop`). The same core drives every runtime, including OpenClaw.
...
- A `tool-call` event whose name is `delegate` (a native leader's structured hand-off) or matches `sessions_send` (OpenClaw's built-in) becomes a single delegation, with the target resolved from the call's arguments (`assignee` / `sessionKey` / `agentId` / `label`) against the team roster.
...
A `<plan>` of ordered `<step>` blocks becomes a durable dependency chain on the board: step _i_ is linked to depend on step _i−1_, so only step 0 starts ready. Each step's intended target is encoded in its task so the chain survives a refresh. When a step completes, the orchestrator runs `pumpReady`, which claims and delivers any step whose blocker just finished, auto-unblock. The board's ready query (`todo`, not dropped, every dependency `done`) and the atomic claim are the final arbiters of double-firing.
...
The depth cap plus report-up-by-default establish a single point where results converge. A sub-task reports to its parent; a top-level task reports to the leader. On every delivery a **child tool blocklist** (`sessions_send`, `sessions_spawn`, `sessions_yield`) stops a delegate from re-delegating and any agent from spawning its own sub-agents. Enforcement is asymmetric: native and coding-runtime drivers strip the tools from the run outright, while the OpenClaw adapter can't block per-message, so Clawboo denies `sessions_spawn` / `sessions_yield` in the Gateway's `tools.deny` on connect (see [AgentSource](/internals/agent-source)). Recursion is bounded by the board's ancestor chain, not by a prompt.
...
- **Server-side, per team.** The engine runs on the server, one instance per team, observing every team member's runtime for structured signals. It is client-independent: closing the browser does not stop a cascade. The server-side executor runner (which runs spawned coding-agent board tasks) is a separate, complementary path.
