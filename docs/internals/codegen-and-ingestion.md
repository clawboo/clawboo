...
So a hand-edit to a generated file, or a forgotten regeneration after a SHA bump, fails the same way in both a PR and a release. Because the verifier imports its render logic from `scripts/lib/ingest-helpers.ts`, the _same_ module the generator uses, the two can never disagree about what "correct" output is.
