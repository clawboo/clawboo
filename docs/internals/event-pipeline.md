...
- **Bridge** (`bridge.ts`): pure parsing. `classifyEvent(frame)` answers "what _is_ this frame?" and returns a `ClassifiedEvent` (`kind`, extracted `agentId`/`sessionKey`, timestamp). It never decides what to do.
- **Policy** (`policy/*`): pure decision-making. `derivePolicy(event)` answers "what _should happen_?" and returns an array of typed `EventIntent`s, a description, not a mutation. It reads only its inputs.
...
The merge has one important rule: **a `runId` change discards the old streaming state**. If an incoming patch carries a different `runId` than the pending one, the queue _replaces_ the pending entry rather than merging, so a stale partial stream from a finished run can never bleed into a fresh run's text.
...
- [The RuntimeAdapter trait](/internals/runtime-adapter), how the _other_ four runtimes emit lifecycle events instead
