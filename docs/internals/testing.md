...
- **Playwright runs in CI but is not yet a _required_ check.** The `e2e` job runs on every pull request and every push to `main`, so a break is visible immediately; the branch ruleset still requires only `Lint`, `Typecheck`, `Test`, and `Build`, so a flake can't wedge every merge while the job earns its reputation. Promoting it is a one-line ruleset change once it has a run history. The clean-install smoke remains the gate on "the assembled artifact works".
