...
<Tip>
Every workspace package has its own `vitest run` test script and its own focused suites: `@clawboo/db`'s board contention and schema tests, `@clawboo/governance`'s verdict and circuit-breaker math, `@clawboo/obs`'s event-projection replay, and so on. `apps/cli` has one too, covering the launcher logic that is risky to get wrong and awkward to exercise end to end: the semver comparison and dev-version guards behind the version-aware attach, the `lsof` / `netstat` parsers behind `clawboo stop`, and the SIGTERM → poll → SIGKILL escalation itself, which is driven through injected dependencies so no test ever signals a real process. Port discovery is split across two files because the two halves need opposite fixtures: `discovery.test.ts` decides what a single port _is_ (Clawboo, access-gated, or something else), binding a throwaway listener and stubbing only the HTTP layer; `discoveryCandidates.test.ts` decides _which ports get tried_ — the `CLAWBOO_API_PORT` pin being the whole search space with no fallback, the runtime `api-port.txt` being a hint that a stale entry falls through, and the 18790-18809 scan reporting nothing when every port is busy. That second file mocks the TCP layer outright, because a real scan would probe the developer's own 18790-18809 and a running Clawboo would answer. The package suites are the unit layer; the `apps/web` two-project config is where the integration and component layers live.
</Tip>
...
- **a second launch reuses the running dashboard**: with a Clawboo-shaped listener already answering and its port recorded in `api-port.txt`, a second spawn of the installed CLI must attach to it — the announced port is the running one, the HTTP signature probe actually ran, and none of the fork-path banners (`Starting Clawboo`, `Dashboard started`) appear. The fake binds an **ephemeral** port, deliberately outside the `18790`–`18809` discovery window, so the ascending scan cannot reach it and a pass proves the runtime-port-file path ran rather than the scan.
...
**Shared connection** (`apps/web/server/lib/__tests__/sharedConnection.test.ts`). The server opens **one** SQLite connection per process and bootstraps the schema **once**, however many requests it serves; before this guard existed, every request handler opened its own connection and re-ran the whole DDL block. The runtime half drives real board handlers against a sandboxed SQLite file and asserts that a burst of 60 reads (and a burst of writes) moves the `dbOpenStats()` counters by exactly zero, plus that tearing down an SSE stream does not close the shared handle. There is no timing assertion: CI shares runners, so a millisecond threshold would flake and get raised until it asserted nothing; the connection count _is_ the property, not a proxy for it. The source half walks `apps/web/server` and asserts zero `createDb(` calls, since the runtime half can only cover the handlers it calls and the background tickers, reaper, supervisor and worktree GC were migrated too. The counters live in the product (`packages/db/src/openStats.ts`) rather than in test scaffolding because no portable alternative exists: `/proc/self/fd` is Linux-only and a better-sqlite3 handle never appears in `process.getActiveResourcesInfo()`.
