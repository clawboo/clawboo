...
| `deriveChatCost`       | `(payload: ChatEventPayload, responseText: string \| null) => ChatCost \| null`                                        | Pure token-spend derivation for a committed turn: real Gateway usage when present, else an output estimate from the response text with `inputTokens: null` for the host to fill in.                       |
...
| `ChatCost`           | type      | Token spend for one committed turn: `{ model, inputTokens: number \| null, outputTokens }`. A `null` `inputTokens` means the Gateway sent no usage block, so the host estimates the prompt from the transcript.                                                                |
| `ChatEventPayload`   | type      | Parsed `chat` frame: `{ runId, sessionKey, state, seq?, stopReason?, message?, errorMessage?, model? }`.                                                                                                                                                                       |
...
| `EventHandlerDeps`   | type      | Injected dependencies for `createEventHandler`: state queries (`getAgentRunId`), dispatchers (`dispatchIntent`, `queueLivePatch`, `appendOutputLines`, `requestHistoryRefresh`, `loadSummarySnapshot`, `refreshHeartbeatLatest`, …), injectable timers, and an optional `log`. |
