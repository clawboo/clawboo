...
| `AgentConfig`         | type       | `z.infer<typeof agentConfigSchema>`, the normalized native-agent shape: identity (`systemPrompt`), routing (`primaryProvider`/`primaryModel`/`fallbacks`/`envVar`), tool toggles (`memory`/`tools`/`tasks`/`teamchat`/`shell`/`custom`, where `shell` gates the native `run_command` tool and is optional: absent reads as off), `participantKind`, `maxTurns`, `budgetUsd`, timestamps, and a dormant `tenantId`. |
...
| `DEFAULT_AGENT_CONFIG`     | `AgentConfig`       | The fallback config when an agent id has no stored config: anthropic + `claude-haiku-4-5`, Memory / Tools / TeamChat on with Tasks `'read'` (a corrupt-blob fallback must never hand a team agent full board writes), `maxTurns: DEFAULT_MAX_TURNS`, `budgetUsd: null`. `shell` is deliberately **absent** here, and absent reads as off, so a shell never arrives switched on.                                                    |
