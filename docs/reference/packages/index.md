...
description: 'The 28 @clawboo/* workspace packages: version, purity, purpose, dependency graph, and build order.'
---
...
There are **28 packages** (23 top-level + 5 nested adapters under `packages/adapters/*`). Versions diverge per package; most sit at `0.1.0`, `events` and `gateway-client` are at `0.1.1`, and `tsconfig` is `0.0.0`.
...
Runtime `dependencies` only (`@clawboo/*` edges). `@clawboo/tsconfig` is a devDependency root, omitted from the runtime graph. Leaf nodes (`config`, `protocol`, `boo-avatar`, `agent-registry`, `capability-registry`, `compaction`, `executor`, `model-catalog`, `obs`, `scheduler`, `worktrees`) have no `@clawboo/*` runtime edges.
...
7. **`apps/web` → `apps/cli`**; the web app consumes all 27 runtime packages (26 as direct dependencies; `boo-avatar` reaches it transitively via `ui`); the CLI consumes only `config` (+ the `tsconfig` devDependency).
