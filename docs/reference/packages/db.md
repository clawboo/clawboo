...
| `createCappedSubtask`                                                                                                 | `(db, parentTaskId, input, caps?) => CreateSubtaskResult`            | Same insert with the per-parent live-child cap (`DEFAULT_MAX_CHILDREN`, 24) and nesting-depth cap (`DEFAULT_MAX_DEPTH`, 2) enforced inside one `BEGIN IMMEDIATE` txn; returns `reason: 'parent_not_found' \| 'child_cap' \| 'depth_cap'` instead of throwing. Backs the Tasks MCP create tools. |
