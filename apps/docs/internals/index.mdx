...
| [AgentSource, registry of record](/internals/agent-source) | How SQLite becomes the source of truth for *who exists*, with the Gateway synced in as one `AgentSource` among many, and the idempotency discipline that keeps the sync honest. |
