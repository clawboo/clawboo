...
---
This page lists the **genuine current limitations** of Clawboo v0.2.0 (verified against commit `03b206a`). Every item is confirmed against current source; these are deliberate deferrals, dormant seams, and runtime-imposed constraints, **not** bugs awaiting a fix. Each entry states the impact and a workaround where one exists.

<Note>
This documents the **v0.2.0 working tree** (commit `03b206a`). The current npm `latest` is **`clawboo@0.1.9`**, so `npx clawboo` installs 0.1.9 until the v0.2.0 tag is published, itself a limitation, [listed below](#4-npx-clawboo-installs-019-not-020).
</Note>
...
- **Connected coding-agent reload trap: fixed.** A user who finishes onboarding with a *connected* non-OpenClaw runtime (a key in the vault) lands back in the dashboard on reload, not the wizard. The fix keys on `onboarded && hasConnectedRuntime`. (The *un*connected SKIP path is a separate, still-open deferral: see [item 6](#6-coding-agent-skip-path-re-onboards-on-reload).)
...
## 4. `npx clawboo` installs 0.1.9, not 0.2.0

The committed CLI is **0.2.0**, but the v0.2.0 git tag is unpushed and 0.2.0 is not published to npm; so npm `latest` is **`clawboo@0.1.9`**. The CLI does not pin a version itself, so `npx clawboo` resolves whatever npm `latest` is.

- **Impact**: `npx clawboo` installs and runs **0.1.9** until the v0.2.0 tag is published. This documentation describes the 0.2.0 working tree; features and routes added after 0.1.9 will not be present in an `npx clawboo` install.
- **Workaround**: to run 0.2.0 today, build from the committed source rather than installing from npm. See [Installation](/getting-started/installation) and the [changelog](/appendices/changelog).

## 5. Codex requires an interactive `codex login` (no API-key connection)
...
## 6. Coding-agent SKIP path re-onboards on reload
...
## 7. No per-process cap on concurrent `/api/obs/stream` subscriptions
...
- **Impact**: a client (or many tabs) opening many concurrent obs streams holds one DB handle + two timers each, with nothing throttling the total. On the local-first single-user model this is fine, but it is a footgun if Clawboo is exposed to many clients.
...
## 8. `buildStructuredSummary` is exported but unwired

The memory package exports `buildStructuredSummary`, a fixed-template compaction helper (Goal → Constraints → Progress → Decisions → Files Touched → Next Steps → Critical Context). It is **intentionally exported and tested but not yet wired into a production caller**: the compaction → `memory_save` relay that would consume it is a tracked follow-up.

- **Impact**: the function exists and works, but nothing in the running system calls it. Do not assume a live consumer when reading the codebase. No user-facing effect.
- **Workaround**: none needed; the template and its contract are kept (and tested) so the eventual wiring is a drop-in. See [Memory](/concepts/memory).
