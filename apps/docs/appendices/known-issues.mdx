...
---
This page lists the **genuine current limitations** of Clawboo v0.2.0 (verified against commit `4aabf2e`). Every item is confirmed against current source; these are deliberate deferrals, dormant seams, and runtime-imposed constraints, **not** bugs awaiting a fix. Each entry states the impact and a workaround where one exists.

<Note>
This documents the **v0.2.0 working tree** (commit `4aabf2e`). The current npm `latest` is **`clawboo@0.1.9`**, so `npx clawboo` installs 0.1.9 until the v0.2.0 tag is published, itself a limitation, [listed below](#4-npx-clawboo-installs-019-not-020).
</Note>
