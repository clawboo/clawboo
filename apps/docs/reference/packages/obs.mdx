...
- **Purpose** The typed substrate every observability surface shares: the orchestration-event Zod schema, the Cursor-style runtime-error taxonomy, the pure graph + fleet-health projection reducers, metric folds, and the adapter-agnostic structured-output judge drive.
- **Workspace deps** none
...
| `isHarnessBug(cls: RuntimeErrorClass): boolean` | True iff `cls === 'Unknown'` — by the Cursor doctrine an unknown class is an alertable harness defect. |
...
| `projectFleetHealth(events, now: number, opts?: FleetHealthOptions): Map<string, AgentHealth>` | Pure, time-sensitive fold into the Gastown triage taxonomy — an agent with an open execution is `working`/`stalled`/`zombie` by quiet time, else `idle`. |
