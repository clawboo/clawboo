...
<Note>
The 304 / 82 counts are the doc-time figures. The catalog's own unit tests assert *lower bounds* (`>= 270` agents, `>= 160` agency, `>= 40` awesome, `>= 15` clawboo) rather than exact counts, so an upstream re-ingest at a newer SHA can grow the catalog without breaking tests. The exact counts above were verified against the committed files at commit `03b206a`.
</Note>
