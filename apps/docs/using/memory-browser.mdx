...
- **Shared-memory banner**: restates that every runtime shares this store and that each runtime *also* keeps a private self-model that is never edited here.
...
1. Type a query into the **Search** input (placeholder *"Search the memory store…"*).
...
- If no provider is reachable, it reads **FTS-only** with an amber note: *"vector / hybrid search degrade to keyword (FTS)."*
