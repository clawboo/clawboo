...
<Note>
This documents the **v0.2.0 working tree** (commit `03b206a`). The current npm `latest` is **`clawboo@0.1.9`**, so `npx clawboo` installs and runs 0.1.9 until the v0.2.0 tag is published. Differences are noted in [Known Issues](/appendices/known-issues).
</Note>
...
<Warning>
**The browser opened to the wrong page (e.g. "Unauthorized").** This was a pre-`0.1.3` bug where the CLI mistook a non-Clawboo listener in the `18790`→`18809` range for the dashboard. The current launcher verifies the Clawboo JSON signature before opening the browser, so it skips Gateway auxiliary ports and Chrome's debug port. If you somehow still hit it, pin a known-free port with `CLAWBOO_API_PORT=<port> npx clawboo`.
</Warning>
