...
<Note>
This documents the **v0.2.0 working tree** (commit `4aabf2e`). The current npm `latest` is **`clawboo@0.1.9`**, so `npx clawboo` installs 0.1.9 until the v0.2.0 tag is published. The native-first onboarding described here is part of v0.2.0; differences are noted in [Known Issues](/appendices/known-issues).
</Note>
