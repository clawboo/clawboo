...
<Note>
This documents the **v0.2.0 working tree** (commit `03b206a`). The current npm `latest` is **`clawboo@0.1.9`**, so `npx clawboo` installs 0.1.9 until the v0.2.0 tag is published. The native-first onboarding described here is part of v0.2.0; see [Known Issues](/appendices/known-issues).
</Note>
...
**Expected result:** you enter the dashboard in **native mode**, no Gateway, no GatewayClient, and land directly in your new team's group chat. You can start chatting with the team immediately.
