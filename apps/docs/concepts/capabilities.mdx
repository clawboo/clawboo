...
It is **not** a marketplace and **not** a skill installer in itself. The [marketplace](/using/marketplace) is where you browse the 304 agents and 82 teams; installing a curated skill is one specific write the inventory routes (to the native source), but the inventory's primary job is to *surface what exists* across heterogeneous runtimes in one consistent shape.
...
| `available` + `diagnostics` | Server-evaluated availability. `available: false` greys the row in *both* renderers; `diagnostics` says why. |
...
| `external-write` | The runtime owns the store; Clawboo writes *through* it (e.g. dialecting an MCP config). | Codex's attached MCP servers (today auth-blocked). |
