...
The historical rule was "SQLite migrations are forward-only, never edit a committed migration file." At commit `4aabf2e` there is **no migration ladder at all**. The schema is created by an inline, idempotent `CREATE TABLE IF NOT EXISTS` DDL block in `createDb()`, declared as the sole source of truth:
