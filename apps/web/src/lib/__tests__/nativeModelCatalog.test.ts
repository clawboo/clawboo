...
describe('nativeModelCatalog', () => {
  it('uses provider-NATIVE model ids (not OpenClaw routing ids)', () => {
    ...
    expect(anthropic?.models.some((m) => m.id === 'claude-sonnet-4-6')).toBe(true)
  ...
  it('nativeLeaderModelFor returns the strongest (index 0) model per provider', () => {
    expect(nativeLeaderModelFor('anthropic')).toBe('claude-sonnet-4-6')
  ...
  it('nativeModelExec maps a catalog id → {provider, model, envVar}', () => {
    expect(nativeModelExec('claude-sonnet-4-6')).toEqual({
    ...
    // an unknown / custom id → null (caller keeps modelTier auto-resolve)
    expect(nativeModelExec('some/unknown-model')).toBeNull()
  })
