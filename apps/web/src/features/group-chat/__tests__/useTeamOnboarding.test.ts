...
describe('useTeamOnboarding', () => {
  ...
  it('markUserIntroduced PATCHes the endpoint with intro text and updates state', async () => {
    ...
    await act(async () => {
      await result.current.markUserIntroduced('Hi I am Alex')
    })
    ...
    expect(result.current.userIntroText).toBe('Hi I am Alex')
    expect(mockFetch).toHaveBeenLastCalledWith('/api/teams/team-1/onboarding', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ userIntroduced: true, userIntroText: 'Hi I am Alex' }),
    })
  })
